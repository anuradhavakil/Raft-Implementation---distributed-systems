	...
	public void run() {
		while (true) {
			...
			//	System.out.println(mgmt.toString());
				...
				}else if(mgmt.hasRaftVote()){//Request for vote received FROM candidates
					ElectionManager.getInstance().processRequest(mgmt);
				}else if (mgmt.hasRaftVoteResponse()){//Responses received TO candidates
					ElectionManager.getInstance().processRequest(mgmt);
				}/**Dhanu*********/
				else if(mgmt.hasLogValue())
				{
					ReplicationManager.processLog(mgmt);
					
				}/***************************/
				else if(mgmt.hasMessage())
				{
					
					logger.info("Detected client message!");
					ReplicationManager.processRequest(mgmt);
