...
import poke.resources.PingResource;
import poke.server.managers.HeartbeatManager;
import poke.server.queue.PerChannelQueue.PerChannelQueueEntry;
	...
	public void run() {
		...
		while (true) {
			...
			try {
				...
				PerChannelQueueEntry msg = sq.inbound.take();

				// process request and enqueue response
				if (msg instanceof PerChannelQueueEntry) {
					PerChannelQueueEntry request = ((PerChannelQueueEntry) msg);
					...
					//PingResource.getInstance().processRequest(request);
					Resource rsc = ResourceFactory.getInstance().resourceInstance(request.req.getHeader());

					Request reply = null;
					if (rsc == null) {
						logger.error("failed to obtain resource for " + request.req);
						reply = ResourceUtil
								.buildError(request.req.getHeader(), PokeStatus.NORESOURCE, "Request not processed");
					} else {
						// message communication can be two-way or one-way.
						// One-way communication will not produce a response
						// (reply).
						reply = rsc.process(request);
					}
