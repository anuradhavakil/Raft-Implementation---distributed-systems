 ...
 * 
 */
public class ServerCommConnection {
	 ...
	 * @param port
	 */
	public ServerCommConnection(String host, int port) {
		this.host = host;
		this.port = port;
	...
	public void sendMessage(poke.comm.Image.Request req) throws Exception {
		// enqueue message
		outbound.put(req);
	}
...
/*	public void addListener(CommListener listener) {
	...
	protected class OutboundWorker extends Thread {
		ServerCommConnection conn;
		boolean forever = true;

		public OutboundWorker(ServerCommConnection conn) {
			this.conn = conn;
		...
		public void run() {
			...
			if (ch == null || !ch.isOpen()) {
				ServerCommConnection.logger.error("connection missing, no outbound communication");
				return;
			}
			...
			while (true) {
				...
				try {
					...
					ServerCommConnection.logger.error("Unexpected communcation failure", e);
					break;
				}
			}

			if (!forever) {
				ServerCommConnection.logger.info("connection queue closing");
			}
		}
	}
	...
	public static class ClientClosedListener implements ChannelFutureListener {
		ServerCommConnection cc;

		public ClientClosedListener(ServerCommConnection cc) {
			this.cc = cc;
		}
