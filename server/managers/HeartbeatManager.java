...
public class HeartbeatManager extends Thread {
	...
	static final int sHeartRate = 2000; // msec
	
	//anu changes
	static int noHBCount = 0;
	...
	public static HeartbeatManager initManager(ServerConf conf,Server server) {
//		System.out.println("\nTODO HB QUEUES SHOULD BE SHARED!\n");
	...
	public int getOutgoingHBsize(){
		return outgoingHB.size();
	}
	
	public int getInComingHBsize(){
	...
	public void processRequest(Management mgmt) {
		...
		if(mgmt.getBeat().getType().equalsIgnoreCase("LEADER")){
		...
		if (req == null){
			noHBCount ++;
			if(noHBCount == 2) {
				logger.info("Start election here...");
			}
			return;
...
//			System.out.println("size from heartbeat: " +outgoingHB.size());
	...
	private Management generateHB() {
		...
		h.setType(server.serverState);
	//	System.out.println("generateHB serverState "+server.serverState);
	...
	public void run() {
		...
		while (forever) {
			try {
				...
				if (outgoingHB.size() > 0 && server.serverState == RaftState.LEADER.toString()) {
	...
	public boolean getTotalHB(){
	//	System.out.println("incoming heartbeat "+incomingHB.size());
